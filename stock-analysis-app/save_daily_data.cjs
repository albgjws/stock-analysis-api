 ...
 *
 * 使用方法：cd 项目目录 && node save_daily_data.cjs
 */
...
async function main() {
  // 读取 data/ 目录下所有 analysis_*.json 文件
  ...
  if (!files.length) {
    console.log('📭 没有 analysis_*.json 缓存文件');
    return;
  }

  // 从文件名提取股票代码：analysis_{code}_{count}_{predictDays}.json
  const codes = [];
  for (const file of files) {
    const m = file.match(/^analysis_(.+?)_\d+_\d+\.json$/);
    if (m) codes.push(m[1]);
  }
  if (!codes.length) { console.log('📭 未能从缓存文件名提取股票代码'); return; }
  ...
  console.log('\n====================');
  console.log('✅ 完成！');
  console.log('   K线:', totalKline, '条');
  console.log('   信号:', totalSignal, '条');
