  ...
  return (
    ...
    >
        ...
        {askReverse.map((a, i) => (
          <div key={`ask-${i}`} style={{ display: 'flex', alignItems: 'center', height: 24, position: 'relative' }}>
            <div style={{
              ...
              width: `${(a.volume / maxVol) * 100}%`,
              background: 'rgba(60, 179, 113, 0.12)',
              borderRadius: 2,
              transition: 'width 0.3s',
            }} />
            <span style={{ width: 30, fontSize: 11, color: '#999', flexShrink: 0 }}>
              卖{i + 5}
            </span>
            <span style={{ flex: 1, fontSize: 13, fontWeight: 600, color: '#3cb371', position: 'relative', zIndex: 1 }}>
              {a.price.toFixed(2)}
            </span>
            <span style={{ width: 70, textAlign: 'right', fontSize: 11, color: '#666', position: 'relative', zIndex: 1 }}>
      ...
      <div style={{
        ...
        fontSize: 18, fontWeight: 700, color: tickColor,
      }}>
        {price?.toFixed(2) || '—'}
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
        {buyDirect.map((b, i) => (
          <div key={`bid-${i}`} style={{ display: 'flex', alignItems: 'center', height: 24, position: 'relative' }}>
            ...
            <span style={{ flex: 1, fontSize: 13, fontWeight: 600, color: '#cf1322', position: 'relative', zIndex: 1 }}>
              {b.price.toFixed(2)}
            </span>
            <span style={{ width: 70, textAlign: 'right', fontSize: 11, color: '#666', position: 'relative', zIndex: 1 }}>
