...
export async function getAnalysis(code: string, options?: { count?: number; predictDays?: number }): Promise<AnalysisResponse> {
...
export async function getQuantitative(code: string): Promise<any> {
  const url = BASE_URL + '/' + encodeURIComponent(code) + '/quantitative';
  const res = await fetch(url);
  if (!res.ok) return null;
  return res.json();
}

export async function getStockProfile(code: string): Promise<any> {
  const url = BASE_URL + '/' + encodeURIComponent(code) + '/profile';
  const res = await fetch(url);
  if (!res.ok) return null;
  return res.json();
}
